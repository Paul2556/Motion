...
export default function AppSnapshotPage() {
  ...
  const estimatedMinutes = Math.ceil(
    ((queue.length + 1) * speechLength) / 60
  );
  return (
      ...
      <div className="flex h-full flex-col px-4 py-4 sm:px-6 lg:px-8 xl:px-10">
        <header className="mb-6 flex flex-col gap-4 rounded-none border border-white/10 bg-[#0f0f0f] p-4 sm:mb-8 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div className="flex items-center gap-3">
            <Logo compact light />
            <span className="text-xs uppercase tracking-[0.18em] text-white/50">DISEC</span>
          </div>
        ...
        <div className="grid flex-1 gap-6 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_480px]">
          <div className="flex h-full flex-col rounded-none border border-white/10 bg-[#121212] p-5 sm:p-6 lg:p-8 shadow-[0_25px_80px_rgba(0,0,0,.35)]">
