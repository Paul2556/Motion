...
Never use all caps subtitles *at all*.
...
Subagents should be used only for heavy read-only tasks (broad codebase search/exploration,
research). Don't delegate edits or writes to subagents in this repo. Subagents can be used for
other things, but minimize that where possible.

## Commands

```
nub run dev       # start Vite dev server
nub run build     # production build (outputs to dist/)
nub run preview   # preview a production build
nub run lint      # eslint . --max-warnings 0 (zero warnings allowed)
vite # only use this when starting the normal vite server, used after every message that changes the code
```

Use `nub` (not `npm`) for all package/script commands in this repo — `nub install` instead of `npm
install`, `nub run <script>` instead of `npm run <script>`, `nubx <pkg>` instead of `npx <pkg>`.
